...
prompt.get(schema, async (err, config) => {
  ...
  console.log('Unregistering, this can take a minute, please wait...');
  ...
  try {
    await axios.post('https://cps.transnexus.com/v1/unregister', {
      serviceProviderCode: serviceProviderCode,
      spcToken: token.token,
      timestamp: timestamp,
    });
  } catch (err) {
    console.error(`Unable to unregister, recieved "${err.response.data.message}"`);
    process.exit(1);
  }

  console.log(`Successfully unregistered service provider code ${serviceProviderCode}.`);
});
